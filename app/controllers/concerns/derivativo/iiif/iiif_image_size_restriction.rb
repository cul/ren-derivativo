# mixin to allow coercion of requested IIIF canvas size to allowable dimensions
# if they are limited
module Derivativo::Iiif::IiifImageSizeRestriction
  ...
  def restricted_use_iiif_size(size_param, region_param, original, max)
    raise Derivativo::Exceptions::UnsupportedRegionError, "Unsupported region: #{region_param}" if region_param == 'featured'
    ...
    unless size.fits_in?(region.maximum_scaled_size(original, max))
      adjust_size!(original, region, size, max)
    end
    size.to_param
  end

  def adjust_size!(original, region, size, max)
