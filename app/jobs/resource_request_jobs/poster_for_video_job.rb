...
module ResourceRequestJobs
  class PosterForVideoJob < ApplicationJob
    include ResourceRequestJobs::ProcessingHelpers

    queue_as :resource_request_poster_for_video

    # Creates a poster for a video resource.
    ...
    private
      ...
      def generate_and_upload(src_file_location, digital_object_uid, options)
        ...
        Derivativo::FileHelper.working_directory_temp_file(file_prefix, file_suffix) do |dst_file|
          # TODO: Eventually add support for specific frame or time selection. Right now it's always the mid-point frame.
