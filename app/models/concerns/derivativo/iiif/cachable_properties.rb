module Derivativo::Iiif::CachableProperties
  ...
  def has_placeholder_image?
    return true if is_configured_placeholder_id(id)
  ...
  def is_configured_placeholder_id(id)
    id.start_with?('placeholder:') &&
      Derivativo::Iiif::CacheKeys::DC_TYPES_TO_PLACEHOLDER_TYPES.has_value?(id.gsub('placeholder:', ''))
  end

  def placeholder_image_type
    return id.gsub('placeholder:', '') if is_configured_placeholder_id(id)

    generic_resource = fedora_get_representative_generic_resource
    return 'unavailable' unless generic_resource.present?
    return Derivativo::Iiif::CacheKeys::DC_TYPES_TO_PLACEHOLDER_TYPES['Closed'] if generic_resource.closed?
    ...
    # If no dc_type is returned, we want to
  ...
  def representative_resource_closed
