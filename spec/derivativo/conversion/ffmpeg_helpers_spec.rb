...
describe Derivativo::Conversion::FfmpegHelpers do
  ...
  let(:ffmpeg_output_args) do
    '-threads 1 -vn -c:a aac -b:a 128k -ar 48000 -ac 2 -af '\
  ...
  describe '.ffmpeg_convert' do
    it 'works as expected' do
      with_auto_deleting_tempfile('dst', '.mp4') do |dst_file|
        described_class.ffmpeg_convert(
          src_file_path: src_file_path, dst_file_path: dst_file.path,
  ...
  describe '.ffmppeg_input_args_string_as_hash' do
    ...
    let(:expected_hash_args) do
      {
        'threads' => '1', 'something1' => 'value1', 'something2' => 'value2',
        'otherthing' => nil, 'something3' => 'value3', 'yetanotherthing' => nil
      }
    end
