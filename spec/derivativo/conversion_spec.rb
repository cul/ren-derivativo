...
describe Derivativo::Conversion do
  describe ".image_to_image" do
    ...
    it 'works as expected' do
      with_auto_deleting_tempfile('dst', '.png') do |dst_file|
        expect(Imogen::Iiif).to receive(:convert).with(Vips::Image, dst_file.path, 'png', region: 'full', size: 'full', rotation: '90', quality: 'color')
        described_class.image_to_image(src_file_path: src_file_path, dst_file_path: dst_file.path, rotation: '90')
      end
    end
  end
  ...
  describe ".with_logged_timing" do
    ...
    it 'outputs the expected test to the Rails log' do
      expect(Rails.logger).to receive(:info).with("some_method converting /path/to/src ...").ordered
      expect(Rails.logger).to receive(:info).with("some_method finished converting /path/to/src to /path/to/dst in #{fake_processing_time} seconds.").ordered
