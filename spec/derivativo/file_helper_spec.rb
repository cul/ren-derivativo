...
describe Derivativo::FileHelper do
  describe '.file_location_to_file_path' do
    ...
    it 'raises an error when given a valid-format location for a non-existent file' do
      expect { described_class.file_location_to_file_path("file:///does/not-exist") }.to raise_error(Derivativo::Exceptions::UnreadableFilePath)
    end

    it 'raises an error when given an unhandled location format' do
      expect { described_class.file_location_to_file_path("mystery:///what/is/this/protocol") }.to raise_error(Derivativo::Exceptions::UnhandledLocationFormat)
    end
  end
