  ...
  describe '.which' do
    context 'in a unix-like environment' do
      before do
        ...
        allow(described_class).to receive(:case_sensitive_file_path).with(
          '/the/second/path', 'ffmpeg'
        ).and_return('/the/second/path/ffmpeg') # positive match case
      end
    ...
    context 'in a windows environment' do
      before do
        stub_const('File::PATH_SEPARATOR', ';') # so that texts run properly regardless of OS running them
        allow(described_class).to receive(:path_values).and_return(
          ['C:\the\first\path', 'C:\the\second\path', 'C:\the\third\path']
        )
        ...
        allow(described_class).to receive(:case_sensitive_file_path).and_return(false) # default negative match case
        allow(described_class).to receive(:case_sensitive_file_path).with(
          'C:\the\second\path', 'ffmpeg.EXE'
        ).and_return('C:\the\second\path\ffmpeg.exe') # positive match case
      end
