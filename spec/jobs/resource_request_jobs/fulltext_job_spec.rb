...
RSpec.describe ResourceRequestJobs::FulltextJob do
  ...
  describe '#perform' do
    before do
      ...
      expect(Hyacinth::Client.instance).to receive(:upload_file_to_active_storage).with(/#{'working_directory'}.+/, 'fulltext.txt').and_return('some-signed-id')
      expect(Hyacinth::Client.instance).to receive(:create_resource).with(digital_object_uid, 'fulltext', 'blob://some-signed-id')
