...
RSpec.describe ResourceRequestJobs::PosterForVideoJob do
  ...
  describe '#perform' do
    before do
      ...
      expect(Hyacinth::Client.instance).to receive(:upload_file_to_active_storage).with(/#{'working_directory'}.+/, 'poster.png').and_return('some-signed-id')
      expect(Hyacinth::Client.instance).to receive(:create_resource).with(digital_object_uid, 'poster', 'blob://some-signed-id')
