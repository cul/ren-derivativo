...
describe Derivativo::Iiif::IiifImageSizeRestriction do
  ...
  let(:controller_class) { Class.new { include Derivativo::Iiif::IiifImageSizeRestriction } }
  let(:controller) { controller_class.new }

  describe ".restricted_use_iiif_size" do
    # subject should always be of format /!?\d+,\d+/
    let(:result) { controller.restricted_use_iiif_size(size, region, original, max) }
    subject { result }
